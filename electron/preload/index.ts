...
import { Server, SSHKey, Group } from '../../src/types/config';
...
contextBridge.exposeInMainWorld('electronAPI', {
  openSSH: (server: Server) => ipcRenderer.send('open-ssh', server),

  // SSH Keys
  getSSHKeys: () => ipcRenderer.invoke('get-ssh-keys'),
  addSSHKey: (key: SSHKey) => ipcRenderer.invoke('add-ssh-key', key),
  removeSSHKey: (name: string) => ipcRenderer.invoke('remove-ssh-key', name),

  // Servers
  getServers: () => ipcRenderer.invoke('get-servers'),
  addServer: (server: Server) => ipcRenderer.invoke('add-server', server),
  updateServer: (id: string, server: Server) => ipcRenderer.invoke('update-server', { id, server }),
